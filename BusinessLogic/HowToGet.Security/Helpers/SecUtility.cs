		...
		internal static void CheckParameter(ref string param, bool checkForNull, bool checkIfEmpty, string invalidChars, int maxSize, string paramName)
		{
			...
			if ((maxSize > 0) && (param.Length > maxSize))
			{
				throw new ArgumentException(
					String.Format(Resources.Parameter_too_long, paramName, maxSize.ToString(CultureInfo.InvariantCulture)), paramName);
			}
			...
			if (!string.IsNullOrWhiteSpace(invalidChars))
			{
				...
				for (int i = 0; i < chars.Length; i++)
				{
					...
					{
						throw new ArgumentException(
							String.Format(Resources.Parameter_contains_invalid_characters, paramName,
							              string.Join("','", invalidChars.Split())), paramName);
					}
				}
			}
		}
